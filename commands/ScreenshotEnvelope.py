...
def screenshotEnvelope(height, width, plateid):
    ...
    try:
        ...
        # Maintain the exact rectangle by:
        #  - using an ORTHOGRAPHIC camera
        #  - setting camera.viewExtents to the rectangle width in cm
        #  - passing an image size whose aspect matches height/width
        ...
        # Up vector along +Y to keep "height" vertical on screen
        cam.upVector = adsk.core.Vector3D.create(0, 1, 0)

        # Set view width in centimeters to exactly match our rectangle width.
        # (The viewport's vertical coverage will be width * out_h/out_w, which equals height_cm if aspect matches.)
        # For an orthographic camera, viewExtents controls the VERTICAL span in world units
        cam.viewExtents = width_cm if width_cm < height_cm else height_cm
